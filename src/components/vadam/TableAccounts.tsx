import {
  ACCOUNTS_VADAM_TABLE,
  COLUMNS_ACCOUNT_VADAM_TABLE,
} from "@/src/constants/listbox";
import {
  Chip,
  Image,
  Table,
  ...
  const renderCell = React.useCallback(
    (account: Account, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "tipo":
          return <p>{account.type}</p>;
        case "status":
          return <Chip>{account.status}</Chip>;
        case "acciones":
          return (
            <div>
              <Tooltip content="Ir al perfil">
                <Image
                  src="/public/images/icons/NextIcon.png"
                  alt="Ir al perfil"
                ></Image>
              </Tooltip>
            </div>
